<template >
   <div class="content-wrapper">
      ...
      <div class="row div-title-module">
        <div class="col-md-10">
          <h2>Editar los datos del usuario [ {{ user.name }} ]</h2>         
        </div>
        <div class="col-md-2">
          <div class="btn-main-view-action">
            <img src="/intranet/img/icons/comprobar.png" class="icon-action-module float-right"  :title="msg_save_update" @click="postUpdate">
            <router-link to="/users-company">
          ...
          <form class="form-basic">       
            <div class="row">
              <div class="form-group col-md-6">
                <label>Rut</label>
                <span class="mdi mdi-alert-circle circle-info" title="El rut del usuario no se puede modificar por razones de seguridad. Comuníquese con soporte para corregir este campo."></span>
                  <input type="text" class="form-control form-control-sm" v-model="user.rut" name="rut" v-rut:live maxlength="12" readonly>  
    ...
    mixins: [Util],
      ...
      postUpdate () {
        ...
        .then(response => {
          ...
          self.errors = {}
          self.$router.push({ path: "/users-company"})
        }, response => {   
          self.loader = false  
          self.profiles.forEach(function(val, ind){
                if (val.id ==  self.user.profile_id) {
