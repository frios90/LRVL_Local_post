<template >
    <div class="content-wrapper">
      ...
      <div class="row div-title-module">
          <div class="col-md-10">
            <h2>Continuar con toma de inventario {{inventory.id}}</h2>          
          </div>
          <div class="col-md-2">
            <div class="btn-main-view-action">
              <img src="/intranet/img/icons/comprobar.png" class="icon-action-module float-right"  title="Presione para guardar la inventarios." @click="finish()">
...
<script>
  ...
  export default {
    ...
    methods: {  
      ...
      getinventory () {
          var get_inventory = false
          if (localStorage.getItem('current_edit_inventory')) {
            var local_inventory = JSON.parse(localStorage.getItem('current_edit_inventory'))
            if (parseInt(local_inventory.id) != parseInt(this.id) ) {
               this.loader = false  
               this.$swal({
                  title             : 'Continuación de edición de inventarios [ ' + local_inventory.id + ' ]',
                  text              : 'Existen datos relaciónados a la edición de otro inventarios y esta no se finalizó. Presiona "Sí" para continuar con la edición de dicho inventarios o presione "No" para eliminar los datos mensionados y comenzar con la edición actual',
                  type              : 'warning',
                  showCancelButton  : true,
                  confirmButtonText : 'Sí',
                  cancelButtonText  : 'No',
                  ...
                  if(result.value) {  
                      this.loader = false
                      this.$router.replace({path: '/inventories/edit/'+local_inventory.id}) 
                      this.id           = this.$route.params.id
                      this.inventory    =  JSON.parse(localStorage.getItem('current_edit_inventory'))                     
                      this.product_list =  JSON.parse(localStorage.getItem('current_edit_detail_inventory')) 
                  } else {
                      var self = this
                      this.$http.get('/get-inventory', {params: {id: self.id}})
                        .then(response => { 
                          self.loader = false 
                          self.inventory    = response.body.inventory
                          self.product_list = response.body.details                          
                          localStorage.setItem('current_edit_inventory', JSON.stringify(self.inventory)); 
                          localStorage.setItem('current_edit_detail_inventory', JSON.stringify(self.product_list)); 
                        }, response => {                 
                            self.$toasted.global.APP_GENERAL_ERROR()
                      })   
                  }
      ...
      changeQtyInput () {
        localStorage.setItem('current_edit_detail_inventory', JSON.stringify(this.product_list));
      },   
      async addItem  () {
        event.preventDefault()
        if (event.keyCode === 13) {
      ...
      deleteItem (key_item) {
        this.$swal({
            ...
            if(result.value) {                      
                ...
                localStorage.setItem('current_edit_inventory', JSON.stringify(this.product_list));                    
            } 
          })                     
      },
      finish () {
        ...
        var data = {
          ...
          guide       : this.inventory.guide,
          guide_date  : this.inventory.guide_date,
          list        : this.product_list
        }
        this.$http.post('/inventory-update', data)
        .then(response => {
