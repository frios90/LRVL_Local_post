   ...
   <div class="content-wrapper">
      ...
      <div class="d-flex div-navegate-page">    
        ...
        <router-link to="/profiles">               
          <p class="text-muted mb-0 hover-cursor">&nbsp;perfiles&nbsp;/&nbsp;</p>             
        </router-link>
      ...
      <div class="row div-title-module">
        <div class="col-md-10">
          <h2>Editar menus del perfil [ {{ profile.name }} ]</h2>         
        </div>
        <div class="col-md-2">
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      getData () {
        var self = this
        this.$http.get('/get-profile', {params: {id: self.profile.id}})
        .then(response => {       
          self.profile.name = response.body.label 
      ...
      getMenus () {
        var self = this
        this.$http.get('/get-menus-format-box-for-profile', {params: {id: self.profile.id}})
        .then(response => { 
          self.loader = false    
      ...
      postStore () {
        ...
        this.loader = true
        this.profile.selected_menus = this.list_to_select    
        var self = this  
        console.log(this.profile)
        this.$http.post('/profile-menus-store', this.profile)
        .then(response => {
            self.$toasted.global.APP_GENERAL_SUCCESS()
            self.errors = {}
            self.$router.push({ path: "/profiles"})
